...
Por defecto se usa `data/live/`. La demo usa `data/demo/` con partidas y mapas inventados. La opción `--data-dir` permite elegir otra ubicación.
...
Al preparar el entorno, pip descarga las dependencias indicadas en `requirements.txt`. tosu se obtiene y ejecuta por separado. Abrir un enlace de descarga lleva al sitio oficial de osu!, donde se aplican sus propias condiciones.
